...
def test_scoring_weights_parse_valid() -> None:
    weights = ScoringWeights.parse(VALID_WEIGHTS)
    assert weights.pitch == 0.35
...
@pytest.mark.parametrize(
    "raw",
    [
        "pitch:0.35,rhythm:0.20,breath:0.15,dynamics:0.10,vibrato:0.10,timbre:0.20",  # sums to 1.1
        "pitch:0.35,rhythm:0.20,breath:0.15,dynamics:0.10,vibrato:0.10",  # missing timbre
        "pitch:oops,rhythm:0.20,breath:0.15,dynamics:0.10,vibrato:0.10,timbre:0.10",  # not a number
        "pitch:0.35;rhythm:0.20",  # wrong separator entirely
    ],
)
def test_scoring_weights_parse_rejects_invalid(raw: str) -> None:
    with pytest.raises(ValueError, match=r".+"):
        ScoringWeights.parse(raw)


def test_load_settings_rejects_malformed_scoring_weights(monkeypatch: pytest.MonkeyPatch) -> None:
    _base_env(monkeypatch, SCORING_WEIGHTS="pitch:0.9,rhythm:0.9")
    with pytest.raises(ValidationError):
        load_settings()
