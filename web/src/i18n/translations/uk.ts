...
export const uk: Translations = {
  ...
  errorAlert: {
    ...
    referencePrepFailed:
      'Підготовка референсу для цієї пісні не вдалася. Спробуйте перезапустити її.',
  ...
  songPrepFailed: {
    body:
      'Референс для цієї пісні не вдалося підготувати, тож її ще не можна проаналізувати. ' +
      'Це може статися, якщо попередня спроба з тим самим аудіо не вдалась. Перезапуск ' +
      'підготовки спробує ще раз.',
    retry: 'Повторити підготовку пісні',
    retryPending: 'Перезапуск…',
  },
  recordingCapture: {
